...
void TilesEditor::populate()
{
  ...
  {
    const amarlon::TileData& tile = _tilesData.tile(i);
...
amarlon::TileData* TilesEditor::getTile(std::function<bool (const amarlon::TileData&)> filter)
{
  for( int i=0; i < _tilesData.tile_size(); ++i )
  {
    const amarlon::TileData& tile = _tilesData.tile(i);
    if ( filter(tile) ) return const_cast<amarlon::TileData*>( &tile );
  }
  return nullptr;
}
...
void TilesEditor::on_tTable_cellDoubleClicked(int row, int /*column*/)
{
  ...
  if ( item )
  {
    ...
    auto* tile = getTile([&](const amarlon::TileData& s){ return s.id() == id; });
    if ( tile )
    {
